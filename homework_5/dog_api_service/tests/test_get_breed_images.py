...
def test_get_breed_images(breed):
    response = requests_session.get(f"{requests_session.url}/breed/{breed}/images")
    assert response.status_code == 200
    validate(instance=response.json(), schema=get_json_schema(file_name='GET_api_breed_breed_images.json'))
